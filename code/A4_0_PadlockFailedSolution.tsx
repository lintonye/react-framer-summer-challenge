...
export function PadlockFailedSolution() {
    const lockControls = useAnimation()
    const textControls = useAnimation()
    async function shakeLock() {
        const spring = {
            type: "spring",
            stiffness: 500,
            damping: 4,
        ...
        lockControls.start({ x: -10, transition: spring })
        await lockControls.start({ scale: 0.17, transition: { delay: 0.5 } })
    }

    async function revealText() {
        await shakeLock()
        lockControls.start({ x: -50 })
